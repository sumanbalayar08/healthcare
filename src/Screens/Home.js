...
import '../App.css'
import { portfolio,counsel,why } from '../infotext/portfolio.js'
...
const Home = () => {
  return (
    <div>
      ...
      <div className='home'>
        <div className='main'>
          <span className='headline'>Meet our Health Counselors</span>
          <p className='paragraph1'>{counsel}</p>
          <div>
            <button className='button'>GET STARTED</button>
          </div>
        </div>
        <div>
          <img src={logo} className='custom-image'/>
        </div>
      </div>

      <div className='home'>
        <div className='main'>
          <span className='headline'>Why HealthCare?</span>
            {why.map((item)=>(
              <ul className='quest'>
                <li className='bullets1'>{item.title}</li>
                <li className='bullets'>{item.desc}</li>
              </ul>
            ))}
        </div>
        <div>
